...
COL_BG  = "#e2e8ee"
CARD_W  = 230
SAVE_PATH = os.path.join(os.path.expanduser("~"), "kanban_board.json")
    ...
    if os.path.exists(SAVE_PATH):
        try:
            with open(SAVE_PATH) as f:
                data = json.load(f)
            for c in COLS:
                board[c] = [Card.from_dict(d) for d in data.get(c,[])]
        except Exception:
            pass
...
def add_card(col):
    ...
    if title and title.strip():
        ...
        board[col].append(Card(title.strip(),body)); save(); refresh()
